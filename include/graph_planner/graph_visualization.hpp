...
class GraphVisualizer {
 public:
  ...
  void vizAgent(CTP::Agent &a, const GraphD &g) { points_pub.publish(toVisualizationMsg(a, g)); }
  ...
  template <typename T>
  void vizPath(const std::vector<int64_t> &path, const T &g, int id = 0, std::string color = "clear blue") {
    path_pub.publish(toVisualizationMsg(path, g, id, color));
  }

  void vizTitle(std::string text) const { vizText(text, 1000, -0.1, 0.5); }
