...
class DepthNode {
 public:
  int depth;
  std::vector<double> q;
  DepthNode(int depth, std::vector<double> q) : depth(depth), q(q) {}

  DepthNode(const std::vector<double> &raw) : depth(raw[0]), q(raw.begin() + 1, raw.end()) {}
...
class SelectiveDensificationGraph : public RDiscGraph {
 ...
 public:
  ...
  virtual double distanceHeuristic(const std::vector<double> &raw1, const std::vector<double> &raw2) const = 0;
...
class DoublingSDG : public SelectiveDensificationGrid {
 public:
  DoublingSDG(int max_depth) { generateGraph(max_depth); };
...
class ConicSDG : public SelectiveDensificationGrid {
 public:
  ConicSDG(int max_depth) { generateGraph(max_depth); };
