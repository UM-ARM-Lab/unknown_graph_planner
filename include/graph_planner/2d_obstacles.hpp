...
class Empty : public Obstacle {
 public:
  bool isValid(const std::vector<double> &q) const override { return true; };

  double distance(const std::vector<double> &q) const override { return std::numeric_limits<double>::infinity(); }
...
class Rect : public Obstacle {
 public:
  ...
  Rect(const Eigen::Vector2d &p1, const Eigen::Vector2d &p2) : x1(p1.x()), y1(p1.y()), x2(p2.x()), y2(p2.y()){};
  ...
  double distance(const std::vector<double> &q) const override {
    ...
    std::vector<double> corner_dists{Norm({rx1, ry1}), Norm({rx1, ry2}), Norm({rx2, ry1}), Norm({rx2, ry2})};
...
class ConvexPolygon : public Obstacle {
 public:
  ...
  ConvexPolygon(const std::vector<Eigen::Vector2d> &points) : points(points) {
    if (!EigenHelpers::CloseEnough(points.front(), points.back(), EDGE_DISCRETIZATION / 2)) {
  ...
  bool isValid(const std::vector<double> &q) const override {
    auto isRightSide = [](const std::vector<double> &q_test, const Eigen::Vector2d &p1, const Eigen::Vector2d &p2) {
...
class Obstacles {
 public:
  ...
  visualization_msgs::MarkerArray toMarkerArray(double z_scale = 0.01, std::string ns = "",
                                                std_msgs::ColorRGBA color = DEFAULT_COLOR) const {
