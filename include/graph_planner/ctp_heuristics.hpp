...
Action POMT(NltpProblem<BeliefGraph> &ctp, double alpha, GraphVisualizer &viz) {
  ...
  auto result = LazySP<std::vector<double>>::PerformLazySP(ctp.belief_graph, ctp.agent.current_node,
                                                           ctp.agent.goal_node, &distanceHeuristic, eval_fun);
  viz.vizPath(result.first, ctp.true_graph);
  return result.first[1];
}
...
Action HSPD(NltpProblem<BeliefGraph> &ctp, GraphVisualizer &viz) {
  auto exploitation = exploitationPath<BeliefGraph>(ctp.belief_graph, ctp.agent.current_node, ctp.agent.goal_node);

  std::cout << "Exploitation path has cost " << exploitation.second << "\n";

  Path exploration_path =
      explorationPath<BeliefGraph>(ctp.belief_graph, ctp.agent.current_node, exploitation.second, viz);
...
class ExplorationState {
 public:
  ...
  double prob;
  double cost;
  ExplorationState(BeliefGraph g, Path path, double cost, double prob) : g(g), prob(prob), path(path), cost(cost){};
};
...
arc_helpers::AstarResult exploitationPath(BeliefGraph &g, Location start, Location goal) {
  ...
  // other changes
  const auto eval_fun = [](GraphD &g, GraphEdge &e) { return e.getWeight(); };

  return LazySP<std::vector<double>>::PerformLazySP(g, start, goal, &distanceHeuristic, eval_fun);
}
...
Path explorationPath(BeliefGraph &g, Location start, double max_cost, GraphVisualizer &viz) {
  ...
  auto cost_compare = [](const ExplorationState<BeliefGraph> &a, const ExplorationState<BeliefGraph> &b) {
