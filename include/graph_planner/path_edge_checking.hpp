...
// Arc Graph
bool forwardLazyCheck(const std::vector<int64_t> &path, HaltonGraph &g, const Obstacles2D::Obstacles &obs) {
  // std::vector<int> path = A_star(start_ind, goal_ind, g);
  ...
  while (i < path.size() - 1) {
    ...
    std::cout << "Edge from point " << path[i] << " is " << (valid ? "valid" : "invalid") << "\n";
    // std::cout << "Edge from point " << i << " is " << e.validity << "\n";
    return false;
  }
  return true;
 ...
 *    Check and updated edge validity
 */
int forwardMove(const std::vector<int64_t> &path, HaltonGraph &g, const Obstacles2D::Obstacles &obs) {
  if (path.size() <= 1) {
    return path[0];
  }
  ...
  if (e.getValidity() == arc_dijkstras::EDGE_VALIDITY::UNKNOWN) {
    auto validity = obs.isValid(e, g) ? arc_dijkstras::EDGE_VALIDITY::VALID : arc_dijkstras::EDGE_VALIDITY::INVALID;
