...
class pqueue {
 ...
 public:
  ...
  bool isEmpty() const { return priority_queue.empty(); }

  std::pair<Priority, Element> top() { return {priority_queue.begin()->first, priority_queue.begin()->second}; }
  ...
  bool contains(Element elem) { return reverse_lookup.count(elem) > 0; }
};
}  // namespace arc_dijkstras
