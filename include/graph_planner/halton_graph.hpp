...
class RDiscGraph : public GraphD {
 public:
  ...
  RDiscGraph(const RDiscGraph& other)
      : GraphD(other), r_disc(other.r_disc), index(flann::Matrix<double>(), flann::KDTreeIndexParams(1)) {
    rebuildKDTree();
  }
  ...
  int64_t addVertexAndEdges(const std::vector<double>& q, double radius) {
    ...
    for (int64_t near_ind : getVerticesWithinRadius(q, radius)) {
      ...
      if (d > radius) {
        std::cerr << "Warning: edge has distance " << d << ", which is larger than allowed radius " << radius << "\n";
      }
  ...
  uint64_t deserializeSelf(
      ...
      const std::function<std::pair<std::vector<double>, uint64_t>(const std::vector<uint8_t>&, const uint64_t)>&
          value_deserializer) {
  ...
  void saveToFile(const std::string& filepath) {
    std::vector<uint8_t> buffer;
    const auto value_serializer_fn = [](const std::vector<double>& value, std::vector<uint8_t>& buffer) {
      using namespace arc_utilities;
      return SerializeVector<double>(value, buffer, SerializeFixedSizePOD<double>);
    };
  ...
  std::vector<double> getToValue(const arc_dijkstras::GraphEdge& e) const { return getNode(e.getToIndex()).getValue(); }
};
  ...
  Grid(int rows, bool eight_connected = true)
    ...
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < rows; j++) {
        std::vector<double> q{(double)i / ((double)rows - 1.0), (double)j / ((double)rows - 1.0)};
        addVertexAndEdges(q);
      }
    }
  }
...
class HaltonGraph : public RDiscGraph {
 public:
  HaltonGraph(int num_vert, double max_edge_dist, int dim = 2) : RDiscGraph(max_edge_dist) {
  ...
  HaltonGraph(const std::string& filepath) : RDiscGraph(filepath){};
