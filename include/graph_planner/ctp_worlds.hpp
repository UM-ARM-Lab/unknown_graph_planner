...
class BctpGraph : public HaltonGraph {
 public:
  ...
  BctpGraph(int num_vert, double max_edge_dist, int dim = 2) : HaltonGraph(num_vert, max_edge_dist, dim){};
...
class CtpPitfall : public CtpGraph {
 public:
  ...
  void updateEdgeProbabilities(std::pair<const arc_dijkstras::GraphEdge, const arc_dijkstras::GraphEdge> es, double p) {
    using namespace arc_dijkstras;
    edge_probabilities[getHashable(es.first)] = p;
    edge_probabilities[getHashable(es.second)] = p;
  }

  std::pair<std::vector<std::string>, std::vector<std::vector<double>>> getEdgeMsgs() {
    using namespace arc_dijkstras;
...
class BctpGrid : public BctpGraph {
 public:
  ...
  Obstacles2D::Rect getObstacle() { return storm; }
