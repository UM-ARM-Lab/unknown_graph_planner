...
std::vector<int> A_star(int start_ind, int goal_ind, const Graph &g) {
  ...
  while (!open_set.empty()) {
    ...
    for (int edge_ind : g.V[current_ind].edge_inds) {
      if (g.E[edge_ind].validity == EDGE_VALIDITY::INVALID) continue;
      ...
      double tentative_g_score = g_score[current_ind] + g.E[edge_ind].weight;
      ...
      f_score[neighbor] = g_score[neighbor] + heuristic_cost_estimate(g.V[neighbor], g.V[goal_ind]);
    }
    // std::cout << "\n";
  }
