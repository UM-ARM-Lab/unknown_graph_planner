...
class ActionNode {
 public:
  ...
  double getCostEstimate() { return summed_cost / visit_count; }
};
...
class Tree {
 public:
  ...
  void deleteSubtree(ActionNode* root) {
    for (StateNode* child : root->children) deleteSubtree(child);
    delete root;
  }
...
class UCT : public MCTS {
 ...
 public:
  ...
  double uctValue(int parent_visits, double child_cost, double child_visits) {
    ...
    return -child_cost + exploration_const * std::sqrt(std::log((double)parent_visits) / (double)child_visits);
  }
...
class UCTH : public UCT {
 public:
  ...
  virtual arc_helpers::AstarResult heuristicPath(const State& s, int from_node) override {
    ...
    return arc_dijkstras::LazySP<std::vector<double>>::PerformLazySP(s_copy.belief_graph, from_node, s.agent.goal_node,
                                                                     &distanceHeuristic, eval_fun);
  }
};
