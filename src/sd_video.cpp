...
/*
 *  This is for making a video for the ICRA submission
 */
...
void setScene1(Obstacles &obs) {
  ...
  auto cp1 =
      std::make_shared<ConvexPolygon>(std::vector<Vector2d>{Vector2d(.4, .6), Vector2d(.4, 1.1), Vector2d(0.9, 1.1)});
...
void vizGraphBuildUp(GraphVisualizer &viz) {
  for (int depth = 0; depth < 8; depth++) {
    {
      ConicSDG g(depth);
      viz.vizGraph(g, "graph");
    ...
    //     SDHaltonGraph g(depth, true);
    //     viz.vizGraph(g, "graph");
    //     arc_helpers::WaitForInput();
    // }
    // {
    //     SDHaltonGraph g(depth, false);
    //     viz.vizGraph(g, "graph");
    //     arc_helpers::WaitForInput();
    // }
  }
...
int main(int argc, char **argv) {
  ...
  viz.vizObstacles(obs, -(double)depth / 3.0);
  arc_helpers::WaitForInput();
