...
DepthNode SelectiveDensificationGraph::getNodeValue(int64_t ind) const { return DepthNode(getNode(ind).getValue()); }
...
double SelectiveDensificationGraph::verticalEdgeCost(const DepthNode &n1, const DepthNode &n2) const { return 0; }
...
int64_t SelectiveDensificationGraph::addVertexAndEdges(int depth, std::vector<double> q) {
  ...
  if (above_ind >= 0) {
    ...
    auto &new_from_edge = addEdgeBetweenNodes(above_ind, new_node_ind,
                                              verticalEdgeCost(DepthNode(getNode(above_ind).getValue()), new_node));
...
int64_t SelectiveDensificationGraph::addVertexAndEdges(DepthNode dn) { return addVertexAndEdges(dn.depth, dn.q); }
...
double DoublingSDG::distanceHeuristic(const std::vector<double> &raw1, const std::vector<double> &raw2) const {
  DepthNode d1(raw1);
  DepthNode d2(raw2);
...
double ConicSDG::distanceHeuristic(const std::vector<double> &raw1, const std::vector<double> &raw2) const {
  DepthNode d1(raw1);
  DepthNode d2(raw2);
  // std::cout << "Calling dist heuristic with depth " << d1.depth << "\n";
  // std::pow(2, d1.depth);
