    ...
    for(int depth=0; depth <= max_depth; depth++)
    {
        for(int i=0; i<numNodesAtDepth(depth, dim); i++)
        {
            ...
            if(i < 2)
            {
                ...
                {
                    addEdgesBetweenNodes(new_node_ind, above_ind,
                                         verticalEdgeCost(DepthNode(depth - 1, qs[i]),
                                                          DepthNode(depth, qs[i])));
                }
...
double IterativeDeepeningHaltonGraph::verticalEdgeCost(const DepthNode &n1, const DepthNode &n2) const
{
    return 1000;
}
