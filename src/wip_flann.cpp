...
int main() {
  std::vector<double> point{0, 1, 2, 3};
  // 0, 1, 2, 4};
  // mat(point.data(), 2, 4);
  ...
  index = flann::Index<flann::L2<double>>(flann::Matrix<double>(point.data(), 1, 4), flann::KDTreeIndexParams(1));
  ...
  std::cout << "Element has value (" << out_point[0] << ", " << out_point[1] << ", " << out_point[2] << ", "
            << out_point[3] << ")\n";
  return 0;
}
